...
public class Player extends Entity {
  ...
  public void tick() {
    ...
    Camera.y =
        Camera.clamp(this.getY() - (Game.HEIGHT / 2), 0, (World.MAP_HEIGHT * 16) - Game.HEIGHT);
  ...
  public void render(Graphics g) {
    if (!playerIsDamaged) {
      if (dir == right_dir) {
        g.drawImage(
            rightPlayer[index], this.getX() - Camera.getX(), this.getY() - Camera.getY(), null);
        if (hasGun) {
          g.drawImage(
              Entity.GUN_RIGHT_ENTITY,
              this.getX() - Camera.getX(),
              (this.getY() - Camera.getY()) + 4,
              null);
        }
      } else if (dir == left_dir) {
        g.drawImage(
            leftPlayer[index], this.getX() - Camera.getX(), this.getY() - Camera.getY(), null);
        if (hasGun) {
          g.drawImage(
              Entity.GUN_LEFT_ENTITY,
              this.getX() - Camera.getX(),
              (this.getY() - Camera.getY()) + 5,
              null);
        }
      } else if (dir == up_dir) {
        g.drawImage(
            upPlayer[index], this.getX() - Camera.getX(), this.getY() - Camera.getY(), null);
        if (hasGun) {
          g.drawImage(
              Entity.GUN_UP_ENTITY,
              (this.getX() - Camera.getX()) + 5,
              this.getY() - Camera.getY(),
              null);
        }
      } else if (dir == down_dir) {
        g.drawImage(
            downPlayer[index], this.getX() - Camera.getX(), this.getY() - Camera.getY(), null);
        if (hasGun) {
          g.drawImage(
              Entity.GUN_DOWN_ENTITY,
              (this.getX() - Camera.getX()) - 5,
              (this.getY() - Camera.getY()) + 4,
              null);
        }
